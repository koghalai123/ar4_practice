...
class AR4Robot(Node):
    ...
    def to_preferred_frame(self, position=None, euler_angles=None, old_reference_frame="base_link", new_reference_frame="base_link"):
        ...
        pitch = transformed_euler[1] + self.angle_offsets["pitch"]  
        roll = transformed_euler[0] + self.angle_offsets["roll"]    
    ...
    def move_to_pose(self, position, orientation_euler=None, orientation_quat=None, 
        ...
        if orientation_quat is not None:
            ...
            self.log_warn("No orientation provided")
            return False
    ...
    def get_ik(self, position, euler_angles, frame_id="base_link", 
        ...
        joint_angles = self.moveit_client.get_ik(transformed_position, transformed_orientation, frame_id, 
                         target_link, avoid_collisions)
        return joint_angles
        ...
        * euler_angles: [roll, pitch, yaw] in radians in the specified frame
        ...
        if isinstance(joint_positions, np.ndarray):
            ...
            if len(joint_positions) != 6:
                self.log_error("Joint positions array must have 6 elements")
                return None, None
            joint_dict = {
            ...
            self.log_error("joint_positions must be dict or numpy array")
            return None, None

        # Create FK service client if it doesn't exist
        if not hasattr(self, 'fk_client'):
            self.fk_client = self.moveit_client.create_client(
                GetPositionFK, 
                'compute_fk'
            )
        ...
        if not self.fk_client.wait_for_service(timeout_sec=5.0):
            self.log_error("FK service not available")
            return None, None
        ...
        if future.result() is not None:
            ...
            if response.error_code.val == response.error_code.SUCCESS:
                ...
                self.log_error(f"FK service failed with error code: {response.error_code.val}")
                return None, None
        else:
            self.log_error("FK service call failed")
            return None, None'''
...
def main():
    ...
    target2 = np.array([0.5, 0.0, 0.5, 0.5, 0.5, 0.5])
    robot.move_to_joint_positions(target2)
