...
def main(args=None):
    ...
    for i in range(10):
        ...
        targetPos = np.array([globalHomePos[0]+0.1, globalHomePos[1]-0.2, 0])
        ...
        marker_publisher.publish_arrow(
            ...
            orientation=np.array([endEffectorOrientWeirdFrame[0], endEffectorOrientWeirdFrame[1]+np.pi/2, endEffectorOrientWeirdFrame[2]]),     # Default orientation (points along x-axis)
            length=np.linalg.norm(vectorToTarget),                         # 30cm long arrow
